    ...
    public void StartNewCycle()
    {
        ...
        if (remainingSlots > 0 && selectedPackNumbers != null && selectedPackNumbers.Count > 0)
        {
            ...
            if (availableGroups.Count > 0)
            {
                ...
                {
                    DataManager.Instance.PlayerData.playedSubEventGroups.Add(selectedPackAndGroup.groupId); // 확정되면 추가
                    PlaythroughHistory.Instance.RecordSubEventGroup(selectedPackAndGroup.groupId); // 다음 회차 분기를 위해 영구 기록
