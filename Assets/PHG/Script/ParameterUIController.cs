...
public class ParameterUIController : MonoBehaviour
{
    ...
    public void InitializeAndDisplayStats()
    {
        ...
        UpdateWarInstantly(GamePlayerStats.Instance.GetStat(ParameterType.전황));
        UpdateKarma(GamePlayerStats.Instance.GetStat(ParameterType.카르마));
    ...
    public void UpdateAffectedToggles(List<ParameterChange> changes, bool isSubEvent = false)
    {
        ...
        foreach (var change in changes)
        {
            ...
            if (GamePlayerStats.Instance != null && GamePlayerStats.Instance.ActiveTrait == CommanderTrait.Wille && change.parameterType == ParameterType.정치력)
                continue;
    ...
    public void UpdatePoliticsLockState()
    {
        ...
        else
        {
            ...
            int currentValue = GamePlayerStats.Instance.GetStat(ParameterType.정치력);
    ...
    private void OnStatChanged(ParameterType type, int changeAmount, int newValue)
    {
        ...
        if (GamePlayerStats.Instance != null && GamePlayerStats.Instance.ActiveTrait == CommanderTrait.Wille && type == ParameterType.정치력)
        {
            return;
        }
